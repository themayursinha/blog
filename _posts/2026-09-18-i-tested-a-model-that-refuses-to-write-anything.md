...
I ran four backends against the same 34 cases. Three were general chat models behind the same interface, so the comparison is not confused by the workflow. Claude Haiku 4.5 is Anthropic's small fast tier. GPT-5.6 Luna is the cheapest model here, at $0.20 per million input tokens. GPT-5.6 Terra is a mid-tier reasoning model at $2 and $12, and TypeSafe's own launch post names Terra as the model closest to Jev in intelligence on System One tasks, which makes it the fairest peer in the set. None of the three is a frontier model, and that is deliberate. A model that competes on cost has to beat the cheap models first, and if it cannot, the frontier comparison is academic.
...
{% include figure.html src="/img/decision-model-tradeoff.svg" label="Fig. 1 · Accuracy against cost" caption="Composed decision accuracy on 34 security cases as raw counts, with the cost of one full pass over each workflow. Jev landed 14 of 20 and 9 of 14 decisions. Haiku landed 16 of 20 and 11 of 14, and Terra, the most expensive model here, landed 18 of 20 and 8 of 14. At these counts one case is 5 points on incidents and 7.1 on traces, so the intervals overlap and the ordering is not a result." alt="Grouped bar chart of composed decision accuracy as counts for Jev 1.13, Claude Haiku 4.5, GPT-5.6 Luna and GPT-5.6 Terra, with cost per workflow run shown separately" %}

## The numbers

Real Jev landed 14 of 20 incident decisions and 9 of 14 trace decisions. Haiku 4.5 landed 16 of 20 and 11 of 14. Luna landed 15 of 20 and 8 of 14. Terra, a mid-tier model costing $2 and $12 per million tokens, landed 18 of 20 and 8 of 14.

Read as counts, the story changes. Jev is **two decisions** behind Haiku on each workflow, out of 20 and 14. The 95 percent Wilson intervals are 48.1 to 85.5 percent for Jev against 58.4 to 91.9 for Haiku on incidents, and 38.8 to 83.7 against 52.4 to 92.4 on traces. Those intervals overlap almost completely.

Terra is what settles the reading. It is the most expensive model in the comparison by a factor of two over Haiku and a hundred over Jev, and it produced the best incident score and the joint worst trace score. Luna, the cheapest model, sits between Jev and Haiku on incidents and below both on traces. There is no ordering here that tracks price, or capability tier, or anything else I can defend at 20 and 14 cases. The earlier version of this post claimed a 10 to 14 point gap between Jev and Haiku, and the counts do not support that. On this sample the accuracy comparison is not a finding.

What does survive is the shape of the trade. Jev costs one forty-seventh of Haiku and one ninety-ninth of Terra, and answers about seven times faster than Haiku, and it produced zero downgrades under adversarial probes while Luna produced nine across the two workflows.

Then the other axis, which is unambiguous. Jev answered in 0.29 and 0.28 seconds on average, against 2.03 and 2.08 seconds for Haiku, so it is about 7 times faster as well as 47 times cheaper. The full 34-case run cost $0.00208 against $0.09765, and that second number is not a projection from a price page. It is measured, and the gateway's own billed figure matched the arithmetic to the cent.
